...
def reverse_one_hot(image_batch):
    """
    Transforms a batch of images in one-hot format to a batch of class-index maps.
    
    # Arguments
        image_batch: A tensor of shape [N, C, H, W], where C is the number of classes.

    # Returns
        A tensor of shape [N, H, W], where each pixel value is the predicted class index.
    """
    # Find the class index with the maximum value along the channel (class) dimension.
    return torch.argmax(image_batch, dim=1)
